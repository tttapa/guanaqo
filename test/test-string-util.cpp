...
TEST(stringutil, split) {
    ...
    EXPECT_EQ(guanaqo::split("abc.defgh", "."), strvwtup("abc", "defgh"));
}

TEST(stringutil, split2) {
    using strvwtup = std::tuple<std::string_view, std::string_view>;
    EXPECT_EQ(guanaqo::split("abc..e.", "."), strvwtup("abc", ".e."));
}

TEST(stringutil, splitEmpty) {
    using strvwtup = std::tuple<std::string_view, std::string_view>;
    EXPECT_EQ(guanaqo::split("abc", "."), strvwtup("abc", ""));
}

TEST(stringutil, splitStringview) {
    using strvwtup = std::tuple<std::string_view, std::string_view>;
    EXPECT_EQ(guanaqo::split("abcdefgh", "cde"), strvwtup("ab", "fgh"));
}
...
TEST(stringutil, splitSecond) {
    using strvwtup = std::tuple<std::string_view, std::string_view>;
    EXPECT_EQ(guanaqo::split_second("abc.defgh", "."),
              strvwtup("abc", "defgh"));
}
...
TEST(stringutil, splitSecondEmpty) {
    using strvwtup = std::tuple<std::string_view, std::string_view>;
    EXPECT_EQ(guanaqo::split_second("abc", "."), strvwtup("", "abc"));
}
...
TEST(stringutil, join) {
    ...
    EXPECT_EQ(guanaqo::join(strings), "abc, de, fghi");
}

TEST(stringutil, join2) {
    std::array strings{"abc", "de", "fghi"};
    EXPECT_EQ(guanaqo::join(strings, {.sep = "<->"}), "abc<->de<->fghi");
}

TEST(stringutil, joinEmpty) {
    std::array<std::string_view, 0> strings;
    EXPECT_EQ(guanaqo::join(strings), "∅");
}

TEST(stringutil, joinEmpty2) {
    std::array<std::string_view, 0> strings;
    EXPECT_EQ(guanaqo::join(strings, {.empty = "[]"}), "[]");
}
...
TEST(stringutil, joinQuote) {
    std::array strings{"abc", "de", "fghi"};
    EXPECT_EQ(guanaqo::join_quote(strings), R"("abc", "de", "fghi")");
}
...
TEST(stringutil, joinQuoteEmpty) {
    std::array<std::string_view, 0> strings;
    EXPECT_EQ(guanaqo::join_quote(strings), "∅");
}

TEST(stringutil, joinQuoteEmpty2) {
    std::array<std::string_view, 0> strings;
    EXPECT_EQ(guanaqo::join_quote(strings, {.empty = "[]"}), "[]");
}
