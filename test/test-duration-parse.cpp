...
TEST(durationparse, multipleFloat) {
    std::chrono::milliseconds v{};
    guanaqo::parse_duration(v, "1.5min+12.5s+13ms");
    EXPECT_EQ(v.count(), 102'513);
}

TEST(durationparse, round) {
    std::chrono::milliseconds v{};
    guanaqo::parse_duration(v, "501µs");
    EXPECT_EQ(v.count(), 1);
}

TEST(durationparse, roundFloat) {
    std::chrono::milliseconds v{};
    guanaqo::parse_duration(v, "0.501ms");
    EXPECT_EQ(v.count(), 1);
}
